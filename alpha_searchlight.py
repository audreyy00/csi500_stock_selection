...
* **轨迹**：爆发日前 ``window`` 个交易日的原子取值序列，对该序列做一元线性回归得到斜率；
  在多只牛股之间汇总 **mean_slope** 与 **consistency** = |mean| / (std + ε)。
* **因子**：对每个通过 consistency 阈值的原子，按窗口滚动斜率乘以 ``direction``
  （由 mean_slope 符号决定），得到 ``alpha_slope_<atom>``。
...
与 XGBoost 衔接：``features.SEARCHLIGHT_SLOPE_COLUMNS`` 与 ``features.build_features`` merge 的列名对齐。
"""
...
class AlphaSearchlight:
    ...
    def prepare_atoms(self) -> list[str]:
        ...
        r5 = ret_1d.groupby(gid).transform(lambda x: x.rolling(5, min_periods=4).std())
        r20 = ret_1d.groupby(gid).transform(lambda x: x.rolling(20, min_periods=15).std())
    ...
    def build_alpha_from_pattern(self, atom_name: str, mean_slope: float) -> str:
        """``alpha_slope_<atom>`` = direction * rolling_slope(atom)。"""
        direction = -1.0 if mean_slope < 0 else 1.0
        alpha_name = f"alpha_slope_{atom_name}"
        ...
        self.df[alpha_name] = slopes * direction

        self.last_valid_alphas.append(alpha_name)
        return alpha_name
    ...
    def run_mining_pipeline(
        ...
        *,
        ...
        print(f">> burst 列 = {burst_col}, window={self.slope_window}, 牛股 {len(self.winners)}")
        for atom in atoms:
            if atom not in self.df.columns:
                warnings.warn(f"跳过缺少列: {atom}", stacklevel=2)
                continue
            ms, cs, nw = self.extract_trajectory_consistency(atom, burst_col=burst_col)
            print(f"   atom={atom!r}: mean_slope={ms:.6g} consistency={cs:.4f} winners_used={nw}")
            if nw == 0 or not np.isfinite(cs) or cs < float(consistency_min):
                continue
            alpha_col = self.build_alpha_from_pattern(atom, ms if np.isfinite(ms) else 0.0)
            valid.append(alpha_col)
...
def _main_argv() -> None:
    ...
    if args.out_parquet is not None:
        ...
        cols += [c for c in eng.df.columns if c.startswith("alpha_slope_")]
